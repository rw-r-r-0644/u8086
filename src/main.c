...
#else
#define KEYBOARD_DRIVER 0
#endif

// Global variable definitions
unsigned char *mem, *io_ports, *opcode_stream, *regs8, i_rm, i_w, i_reg, i_mod, i_mod_size, i_d, i_reg4bit, raw_opcode_id, xlat_opcode_id, extra, rep_mode, seg_override_en, rep_override_en, trap_flag, int8_asap, scratch_uchar, io_hi_lo, *vid_mem_base, spkr_en, bios_table_lookup[20][256];
...
int Menu_Main()
{
	...
	disk[2] = open("sd:/bios", O_RDWR);
	disk[1] = open("sd:/fd.img", O_RDWR);
	disk[0] = 0;
	...
	for (; opcode_stream = mem + 16 * regs16[REG_CS] + reg_ip, opcode_stream != mem;)
	{
		...
		printf("loop %02X\n", xlat_opcode_id); // this is stuck at 0...there must be some read error or smth
			...
			OPCODE 48: // Emulator-specific 0F xx opcodes
				switch ((char)i_data0)
				{
					OPCODE_CHAIN 0: // PUTCHAR_AL
						write(1, regs8, 1)
					OPCODE 1: // GET_RTC
						// TODO: RE __OSCalendarTimeToPOSIXTime
						/*time(&clock_buf);
						ftime(&ms_clock);
